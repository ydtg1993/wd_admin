...
class MovieCategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $res = MovieCategory::orderBy('sort', 'desc')->orderBy('updated_at', 'desc')->paginate($request->get('limit', 30));
        $records = $res->toArray();
    ...
    public function create(Request $request)
    {
        ...
        try {
            ...
            $id = MovieCategory::insertGetId(['name'=>$data['name'],
                'status'=>$data['status'],
                'show'=>$data['show'],
                'sort'=>$data['sort']]);
            $text = base64_encode(trim($data['name']));
            RedisCache::addSet('moive_category',$id,$text);
    ...
    public function edit(Request $request, $id)
    {
        ...
        $data = $request->all();
        try {
            MovieCategory::where('id',$id)->update(['status'=>$data['status'],'show'=>$data['show'],'sort'=>$data['sort']]);
