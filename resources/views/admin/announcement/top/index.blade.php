    ...
    <div class="layui-card">
        <div class="layui-card-header layuiadmin-card-header-auto">
            <div class="layui-btn-group ">
                @can('announcement.top.create_top')
                    <a class="layui-btn layui-btn-sm" href="{{ route('admin.top.create_top') }}">添 加</a>
                @endcan
            </div>
        </div>
        ...
        <div class="layui-card-body">
            ...
            <script type="text/html" id="options">
                <div class="layui-btn-group">
                    @can('announcement.top.edit_top')
                        <a class="layui-btn layui-btn-sm" lay-event="edit">编辑</a>
                    @endcan
                    @can('announcement.top.destroy_top')
        ...
        <script>
            layui.use(['layer', 'table', 'form'], function () {
                ...
                var dataTable = table.render({
                    ...
                    , autoSort: false
                    , height: 500
                    , url: "{{ route('admin.top.data') }}"+"?type=1" //数据接口
                    , page: true //开启分页
                    , cols: [[ //表头
                ...
                table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if (layEvent === 'del') {
                        layer.confirm('确认删除吗？', function (index) {
                            layer.close(index);
                            var load = layer.load();
                            $.post("{{ route('admin.top.destroy_top') }}", {
                                _method: 'delete',
                                id: data.id
                            }, function (res) {
                                layer.close(load);
                        ...
                        location.href = '/admin/top/' + data.id + '/edit_top';
                    }
                });
                //搜索
