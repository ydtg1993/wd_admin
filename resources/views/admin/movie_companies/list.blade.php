        ...
        <script>
            layui.use(['layer', 'table', 'form','laydate'], function () {
                ...
                var dataTable = table.render({
                    ...
                    ,id:'table'
                    , height: 500
                    , url: "{{ route('admin.movie.companies.list') }}" //数据接口
                    , method:'POST'
                    , page: true //开启分页
                    , cols: [[ //表头
                        {field: 'id', title: 'ID', sort: true, width: 80}
                        , {field: 'company_id', title: '片商ID'}
                        , {field: 'company', title: '片商名字'}
                        , {field: 'number', title: '影片番号'}
                        , {field: 'name', title: '名称'}
                        , {field: 'small_cover', title: '封面'}
                        , {field: 'release_time', title:'发布日期'}
                ...
                table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if (layEvent === 'del') {
                        layer.confirm('真的删除行 '+data.id, function(index){
                            $.ajax({
                                url:'{{route('admin.movie.companies.list')}}',
                                method:'delete',
