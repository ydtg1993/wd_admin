...
<link rel="stylesheet" href="/bootstrap/css/bootstrap.css">
    <div class="layui-card">
        ...
        <div class="layui-card-body">
            <form class="layui-form" action="{{route('admin.movie.label.create')}}" method="post">
                {{ method_field('post') }}
                {{csrf_field()}}
...
@section('script')
    ...
    <script>
        layui.use(['element','form'],function () {
            ...
            //分类
            componentSelect('category',[],JSON.parse('<?=json_encode(str_replace("'","",$categorys));?>'));
            //子标签
            componentSelect('children',[],JSON.parse('<?=json_encode(str_replace("'","",$childrens));?>'));
        });
    </script>
@endsection
